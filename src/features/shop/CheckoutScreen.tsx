...
export const CheckoutScreen = ({ navigation }: CheckoutScreenProps) => {
  const { cart, checkout, clearCart } = useShopStore();
  ...
  const discount = (promoEligible || referralEligible) ? subtotal * 0.10 : 0;
  ...
  const handlePlaceOrder = async () => {
    ...
    // Simple payment validation for card entries
    ...
    try {
      // In a real app, integrate Stripe/Apple Pay here before checkout()
      const promoToSend = promoEligible ? promoCode.trim().toUpperCase() : (referralEligible ? 'REFERRAL10' : undefined);
      await checkout(promoToSend);
      ...
      return (
        <View style={styles.paymentDetails}>
          <TouchableOpacity
            ...
            style={[styles.applePayButton, isProcessing && styles.disabledButton]}
          >
          ...
          <Text style={styles.paymentNote}>
            Apple Pay requires iOS and merchant setup. Integrate Stripe/Apple Pay for production.
          </Text>
        </View>
      );
    }
    if (selectedPayment === 'paypal') {
      return (
        <View style={styles.paymentDetails}>
          ...
          >
            <Text style={styles.paypalText}>{isProcessing ? 'Processing...' : 'Pay with PayPal'}</Text>
          </TouchableOpacity>
          <Text style={styles.paymentNote}>
            PayPal flow is mocked. Connect your gateway to enable.
          </Text>
        </View>
      );
    }
      ...
      >
        ...
        <View style={styles.section}>
          ...
          <View style={styles.orderSummary}>
            ...
            {discount > 0 && (
              <View style={styles.summaryRow}>
                <Text style={styles.summaryLabel}>{promoEligible && !referralEligible ? 'Promo Discount (10%)' : 'Referral Discount (10%)'}</Text>
        ...
        <View style={styles.section}>
          ...
          <View style={{ flexDirection: 'row', gap: 12, alignItems: 'center' }}>
            ...
            />
            {!isPromoApplied ? (
              <TouchableOpacity onPress={handleApplyPromo} style={styles.promoButton}>
                <Text style={styles.promoButtonText}>Apply</Text>
              </TouchableOpacity>
            ) : (
              <TouchableOpacity onPress={handleClearPromo} style={styles.promoButton}>
